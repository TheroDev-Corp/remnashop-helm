...
msg-notifications-expiry-reminder =
    <b>⏰ Напоминания о продлении</b>

    <blockquote>
    • <b>Дни до окончания</b>: { $days }
    • <b>Проверка по базе</b>: { $fallback_enabled ->
        [1] 🟢 Включена
        *[0] 🔴 Выключена
    }
    </blockquote>

    Напоминания приходят из вебхуков Remnawave (за 3, 2 и 1 день), а проверка по базе раз в час досылает их, если вебхуки не настроены или не дошли. Каждое напоминание уходит один раз.

    Для дней больше 3 действует переключатель «3+ дня» в пользовательских уведомлениях.

msg-notifications-expiry-reminder-days =
    <b>📅 Дни напоминаний</b>

    Текущее значение: <b>{ $days }</b>

    Введите, за сколько дней до окончания подписки напоминать — через пробел или запятую (от 1 до 30, не больше 5 значений). Например: <code>7 3 1</code>
